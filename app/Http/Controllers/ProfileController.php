...
class ProfileController extends Controller
{
    ...
    protected function buildProfile(Request $request, $user)
    {
        ...
        if (! $loggedIn) {
            ...
            $owner = Auth::id() === $user->user_id;
            $is_following = (!$owner && Auth::check()) ? $user->followedBy(Auth::user()->profile) : false;

            if ($isPrivate || $isBlocked) {
                $requested = Auth::check() ? FollowRequest::whereFollowerId(Auth::user()->profile_id)
                    ->whereFollowingId($user->id)->exists() : false;
        ...
        // Contadores direto do banco (ignora cache desatualizado)
        $postsCount = Status::whereProfileId($profile->id)
            ->whereIn('scope', ['public', 'unlisted'])
            ->whereNull('reblog_of_id')
            ->whereIn('type', ['photo', 'photo:album', 'video', 'video:album', 'photo:video:album'])
            ->count();

        $followersCount = \DB::table('followers')->where('following_id', $profile->id)->count();
        $followingCount = \DB::table('followers')->where('profile_id', $profile->id)->count();

        // Posts do grid: fotos e carrosséis (incluindo carrosseis com video)
        $statuses = Status::whereProfileId($profile->id)
            ->whereIn('scope', ['public', 'unlisted'])
            ->whereNull('reblog_of_id')
            ->whereIn('type', ['photo', 'photo:album', 'photo:video:album'])
            ->orderByDesc('id')
            ->take(30)
            ->get();

        // Vídeos solo para a aba Tube
        $tubeStatuses = Status::whereProfileId($profile->id)
            ->whereIn('scope', ['public', 'unlisted'])
            ->whereNull('reblog_of_id')
            ->whereIn('type', ['video', 'video:album'])
            ->orderByDesc('id')
            ->take(30)
            ->get();
    ...
    protected function getCachedUser($username, $withTrashed = false)
    {
        ...
        return Cache::remember('pfc:cached-user:'.$hash, ($withTrashed ? 14400 : 900), function () use ($username, $withTrashed) {
            if (! $withTrashed) {
                return Profile::whereNull(['domain', 'status'])->whereUsername($username)->first();
            } else {
                return Profile::withTrashed()->whereNull(['domain', 'status'])->whereUsername($username)->first();
            }
        });
    }
    ...
    protected function privateProfileCheck(Profile $profile, $loggedIn)
    {
        ...
        if ($user->id == $profile->id || ! $profile->is_private) return false;
        return ! Follower::whereProfileId($user->id)->whereFollowingId($profile->id)->exists();
    }
    ...
    public function showAtomFeed(Request $request, $user)
    {
        ...
        $aiCheck = Cache::remember('profile:ai-check:spam-login:'.$profile['id'], 3600, function () use ($profile) {
            ...
            if (! $uid) return true;
            return AccountInterstitial::whereUserId($uid->id)->where('is_spam', 1)->count() ? true : false;
        });
        ...
        $enabled = Cache::remember('profile:atom:enabled:'.$profile['id'], 86400, function () use ($profile) {
            ...
            return $settings ? $settings->show_atom : false;
        });
        ...
        $data = Cache::remember('pf:atom:user-feed:by-id:'.$profile['id'], 14400, function () use ($pid, $profile) {
            $items = Status::whereProfileId($pid)
                ...
                ->take(10)
                ->get()
                ->map(fn($s) => StatusService::get($s->id, true))
                ->filter(fn($s) => $s && isset($s['account'], $s['media_attachments']) && count($s['media_attachments']))
                ->values();
        ...
        abort_if(! $data || ! isset($data['items'], $data['permalink']), 404);
    ...
    public function embed(Request $request, $username)
    {
        ...
        $aiCheck = Cache::remember('profile:ai-check:spam-login:'.$profile->id, 3600, function () use ($profile) {
            return AccountInterstitial::whereUserId($profile->user_id)->where('is_spam', 1)->count() ? true : false;
        });
        if ($aiCheck) {
            return response($res)->withHeaders(['X-Frame-Options' => 'ALLOWALL']);
        }
    ...
    public function stories(Request $request, $username)
    {
        ...
        $exists = Story::whereProfileId($pid)->whereActive(true)->exists();
